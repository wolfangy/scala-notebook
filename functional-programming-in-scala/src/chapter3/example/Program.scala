//package fpinscala.chapter3.example
...
object Program{
  ...
  @main def main: Unit =
    ...
    val result2 = List.init(List(1, 2, 3, 4, 5))
    println(s"init of List(1, 2, 3, 4, 5) is: ${List.map(result2)(_.toString)}")
    println(s"init of List(1, 2, 3, 4, 5) is: ${List.foldRight(result2, "")((cur, acc) => acc + cur.toString + ", ")}")
    println(s"init of List(1, 2, 3, 4, 5) is: ${List.foldLeft(result2, "")((cur, acc) => acc + cur.toString + ", ")}")
}